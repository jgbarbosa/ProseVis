...
public class Visualization extends JPanel implements ComponentListener {
	private final Color davidDataColor[] = {Color.RED, Color.BLUE, Color.GREEN, 
									   Color.ORANGE, Color.GRAY, Color.YELLOW, 
	...
	private void drawColumnColor(Graphics2D g) {
		...
		while(lineNode != null) {
			...
			// Display the corresponding level number, ie, line number or paragraph no. or section no, etc
				TextLayout lineLevelText = new TextLayout(count + " ", scaledFont, g.getFontRenderContext());
				double levelWidth = lineLevelText.getAdvance();
				DisplayBlock displayLevelBlock = new DisplayBlock(levelWidth, this, this.ctr, lineLevelText);
				displayLevelBlock.setBounds(new Rectangle((int)0, (int)(yPos-lineHeight), (int)levelWidth, (int)lineHeight));
			...
			Rectangle2D lineArea = new Rectangle2D.Double(xPos, yPos-lineHeight, ctr.vizWidth, lineHeight*2);
			...
			if(lineArea.intersects(viewArea)){
				...
				while (wordNode != null) {
					...
					if(ctr.renderText){
						...
						textToRender = new TextLayout(text + " ", scaledFont, g.getFontRenderContext());
						tokenWidth = textToRender.getAdvance();
					}else{
						tokenWidth = wordNode.getSyllableCount() * (ctr.zoom * 10.0);
					}

					DisplayBlock block = new DisplayBlock(wordNode, this, this.ctr, tokenWidth, textToRender);
					block.setBounds(new Rectangle((int)xPos, (int)cyPos, (int)tokenWidth, (int)lineHeight));
 ...
 * 
			yPos += lineHeight;
			// Leave a blank line
			TextLayout blankLineText = new TextLayout(" ", scaledFont, g.getFontRenderContext());
			DisplayBlock displayBlankLineBlock = new DisplayBlock(levelWidth, this, this.ctr, blankLineText);
			displayBlankLineBlock.setBounds(new Rectangle((int)0, (int)(yPos-lineHeight), (int)levelWidth, (int)lineHeight));
