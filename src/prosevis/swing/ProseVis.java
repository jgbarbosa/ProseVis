...
public class ProseVis extends JPanel implements ActionListener, ItemListener {
  ...
  public ProseVis() {
    ...
    vizPane = new VizController(JSplitPane.HORIZONTAL_SPLIT, leftVizPanel,
        rightVizPanel, viz1, viz2);
    ...
    JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, vizPane,
        toolPanel);
    ...
    if (OPEN_FILE_COMMAND.equals(command)) {
      ...
      if (vizPane.getNumFiles() < 2) {
        ...
        try {
          ...
          System.err.println("Couldn't find the last path you loaded from.");
        }
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
          ...
          // If a file is already open, we pass it's variable lists to
          // the new file
          if (vizPane.noFiles()) {
        ...
        JOptionPane
            .showMessageDialog(
                this,
                "There are already two files open. To open a new file, please close one of the current files.");
      }
       ...
       * else if (LINENO_VIEW_COMMAND.equals(command)) { e.getActionCommand()
       * System.out.println("came here"); viz1.setViewLineNumber(true);
  ...
  private static void createAndShowGUI() {
    ...
    try {
      ...
      System.err
          .println("Caught an exception related to the look and feel customization:");
  ...
  public void updateViz(boolean search) {
    // Search results are discarded after additional updates to the data
    // panel
    if (!search) {
      vizPane.killSearch();
    }
