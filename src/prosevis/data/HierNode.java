...
public class HierNode extends ProseNode {
  ...
  private int nodeNumber;
	...
	public HierNode(boolean preWord, int ordinal){
  ...
  public void findNode(int currLevel, int reqLevel, int ordinal, HierNodeWrapper result) {
    ...
    throw new RuntimeException("Tried to find node that doesn't exist in this tree");
  }

  private int getMinAtLevel(int currLevel, int reqLevel) {
    if (currLevel < reqLevel) {
      return ((HierNode)children.get(0)).getMinAtLevel(currLevel + 1, reqLevel);
    }
  ...
  private int getMaxAtLevel(int currLevel, int reqLevel) {
    if (currLevel < reqLevel) {
      return ((HierNode)children.get(children.size() - 1)).getMaxAtLevel(currLevel + 1, reqLevel);
    }
    return this.nodeNumber;
  }
}
