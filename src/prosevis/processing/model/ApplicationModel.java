...
public class ApplicationModel {
    ...
    private boolean useGaussianFilter = true;
    private boolean allowSelfSimilarity = true;
    private boolean normalizeEntireDomain = true;
    private boolean normalizeComparison = true;
    private final ToolTipFields tooltipFields = new ToolTipFields();
    private boolean slidesTogether = false;
    ...
    public synchronized void addData(Document newTree, TypeMap correspondingTypeMap) {
        ...
        view.setSmoothingWindow(smoothingWindow, allowSelfSimilarity, useGaussianFilter, normalizeEntireDomain, normalizeComparison);
        data.add(view);
    ...
    public synchronized void setScreenX(int width) {
        xResolution = width;
        geoModel.setX((data.size() > 0) ? xResolution / data.size() : xResolution);
    }
    ...
    public synchronized void setScreenY(int height) {
        yResolution = height;
        geoModel.setY(yResolution);
    }
    ...
    public synchronized int setSmoothingWindow(int window) {
        if (window >= 1 && smoothingWindow != window) {
            ...
            for (DataTreeView d : data) {
                d.setSmoothingWindow(smoothingWindow, allowSelfSimilarity, useGaussianFilter, normalizeEntireDomain, normalizeComparison);
            }
        }
        return smoothingWindow;
    }
    ...
    /**
     * @return the slidesTogether
     */
    public synchronized boolean isSlidesTogether() {
        return slidesTogether;
    }

    /**
     * @param slidesTogether the slidesTogether to set
     */
    public synchronized void setSlidesTogether(boolean slidesTogether) {
        this.slidesTogether = slidesTogether;
    }

    /**
     * @return the useGaussianFilter
     */
    public synchronized boolean getUseGaussianFilter() {
        return useGaussianFilter;
    }

    /**
     * @param useGaussianFilter the useGaussianFilter to set
     */
    public synchronized void setUseGaussianFilter(boolean useGaussianFilter) {
    ...
    /**
     * @return the normalizeEntireDomain
     */
    public synchronized boolean getNormalizeEntireDomain() {
        return normalizeEntireDomain;
    }

    /**
     * @param normalizeEntireDomain the normalizeEntireDomain to set
     */
    public synchronized void setNormalizeEntireDomain(boolean normalizeEntireDomain) {
        this.normalizeEntireDomain = normalizeEntireDomain;
        for (DataTreeView d : data) {
            d.setSmoothingWindow(smoothingWindow, allowSelfSimilarity, useGaussianFilter, normalizeEntireDomain, normalizeComparison);
        }
    }
    ...
    /**
     * @return the normalizeComparison
     */
    public synchronized boolean getNormalizeComparison() {
        return normalizeComparison;
    }

    /**
     * @param normalizeComparison the normalizeComparison to set
     */
    public synchronized void setNormalizeComparison(boolean normalizeComparison) {
        this.normalizeComparison = normalizeComparison;
        for (DataTreeView d : data) {
            d.setSmoothingWindow(smoothingWindow, allowSelfSimilarity, useGaussianFilter, normalizeEntireDomain, normalizeComparison);
        }
    }
